...
func RandLowercase(len int) string {
...
func RandUppercase(len int) string {
	var container string
	var str = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	b := bytes.NewBufferString(str)
	length := b.Len()
...
func RandSleep(millisencond int) {
	ms := millisencond + crand.Intn(millisencond)
...
func RandomUA() string {
	...
	return userAgent[crand.New(crand.NewSource(time.Now().Unix())).Intn(len(userAgent))]
}
